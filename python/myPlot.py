...
def scatter2D(x, colors, markers, n_colors=10, title='None'):
    ...
    ax.scatter(x[markers == 0, 0], x[markers == 0, 1], lw=0, s=30,
               c=palette[colors[markers == 0].astype(np.int)], marker='o')
    sc = ax.scatter(x[markers == 1, 0], x[markers == 1, 1], lw=0, s=60,
                    c=palette[colors[markers == 1].astype(np.int)], marker='*')
    plt.xlim(-25, 25)
    plt.ylim(-25, 25)
    ax.axis('off')
    ax.axis('tight')
