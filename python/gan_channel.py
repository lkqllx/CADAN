...
out_spk_lbs = out_spk_lbs.tolist()
                      # Select n_tst_spks/2 male speakers
for m_spk in range(int(n_tst_spks/2)):
...
def main():
    ...
    D_loss= tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit, labels=sex_X))
    ...
    C_solver = tf.train.AdamOptimizer(0.000001).minimize(C_loss, var_list=[theta_C])
    G_C_solver = tf.train.AdamOptimizer(0.00001).minimize(G_loss_C, var_list=theta_G_C)
    G_D_solver = tf.train.AdamOptimizer(0.00001).minimize(G_loss_D, var_list=theta_G_D)
    sess = tf.Session()
            ...
            if it < 100:
                for i in range(1):
                    _, G_loss_D_curr = sess.run([G_D_solver, G_loss_D], feed_dict={X: x, sex_X:y_sex_1h, spk_label: y_spk_1h,
                                                                           train_flag: 1})
                for i in range(1):
                    _, G_loss_C_curr = sess.run([G_C_solver, G_loss_C],
                                                feed_dict={X: x, sex_X: y_sex_1h, spk_label: y_spk_1h,
                                                           train_flag: 1})
            else:
                for i in range(8):
                    _, G_loss_C_curr = sess.run([G_C_solver, G_loss_C],
    ...
    fig_loss, ax = plt.subplot()
    ax.plot(loss_list)
    plt.show()

    sio.savemat('data/sre12_all_male.mat',
                {'w': all_male, 'spk_logical': all_male_spk_lbs})
    sio.savemat('data/sre12_all_female.mat',
                {'w': all_female, 'spk_logical': all_female_spk_lbs})

    sre12_trn_enc = sess.run(G_log_prob, feed_dict={X: x_trn, train_flag: 1})
